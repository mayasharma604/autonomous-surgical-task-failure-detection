...
# 1. Filter for ONLY the active cutting phase (3_resect)
...
# 2. Sort by filename to ensure we are looking at time-sequence
resect_df = resect_df.sort_values(by=['phase', 'filename'])
resect_df['frame_index'] = range(len(resect_df))

# 3. Calculate Correlation (Are values generally increasing?)
...
if correlation > 0.7:
    ...
    print("⚠️ Weak Increase: The trend is positive but contains significant noise.")
else:
